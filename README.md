...
`/writing/marginalia/` 收录叙事细读、秘封札记、听歌随笔与校订手记。首页、写作页、书架和档案检索均有入口；分类筛选支持键盘操作，未启用 JavaScript 时仍可阅读全部条目。

- 正文位于 `src/content/blog/`，使用 `kind: writing` 和 `阅读边注` 标签。
- 栏目索引、作品资料和参与署名位于 `src/data/marginalia.ts`；`id` 与正文文件名一致。文章页根据索引提供系列导航。
- `MarginaliaFeature.astro` 提供三则精选入口，`ReadingCollaboration.astro` 展示评读与校订案例。
